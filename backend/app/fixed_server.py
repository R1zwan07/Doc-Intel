...
def generate_clean_summary(page_texts: List[Dict], page_count: int, filename: str) -> str:
    ...
    # Header
    summary.append("=" * 70)
    summary.append(f"[BOOK] DOCUMENT SUMMARY: {filename}")
    summary.append("=" * 70)
    summary.append(f"[PAGES] Total Pages: {page_count}")
            ...
            if 30 < len(line) < 400:
                ...
                if any(kw.lower() in clean_line.lower() for kw in important_keywords):
                    if len(clean_line) > 40:
                        key_points.append(f"[KEY] {clean_line}")
        ...
        for img in page.get_images():
            try:
                ...
                if pix.n - pix.alpha < 4:
                    img_data = pix.tobytes("png")
                    diagrams.append({
                        "title": f"[IMG] Figure from Page {page_num + 1}",
                        "type": "image",
...
async def export_markdown():
    ...
    content = f"""# [BOOK] Document Summary: {document_store['filename']}
...
async def export_txt():
    ...
    content = f"""
{'='*70}
[BOOK] DOCUMENT SUMMARY: {document_store['filename']}
{'='*70}
Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
Total Pages: {document_store['page_count']}
{'='*70}
...
async def upload_pdf(file: UploadFile = File(...)):
    ...
    try:
        ...
        print(f"[OK] Extracted {len(pdf_data['text'])} chars, {len(document_store['key_points'])} key points")
